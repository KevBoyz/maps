...
def run_gd_armijo():
    ...
    for k in range(MAX_ITER):
        ...
        if np.linalg.norm(g) <= TOL:
            break
        d = -g
        alpha, step_nf = armijo(x, g, d);  nf += step_nf
        x = x + alpha * d
...
def plot_convergence(results, labels):
    ...
    axes[0].set(title='f(xk) versus iteração', xlabel='Iteração', ylabel='f(xk)')
    axes[0].legend();  axes[0].grid()
    axes[1].set(title='||∇f(xk)|| versus iteração', xlabel='Iteração', ylabel='Norma do gradiente')
...
def plot_trajectory(traj, title, filename):
    ...
    ax.plot(hx, hy, '-o', markersize=4, linewidth=2, color='white', label='Trajetória')
    ax.scatter(hx[0],  hy[0],  s=100, color='cyan', zorder=5, label='Ponto Inicial')
    ax.scatter(hx[-1], hy[-1], s=100, color='lime', zorder=5, label='Ponto Final')
...
if __name__ == '__main__':
    ...
    plot_trajectory(res_gd['traj'],     'Gradiente Descendente - Armijo', 'traj_gd_armijo.png')
